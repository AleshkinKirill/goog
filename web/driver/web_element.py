...
from selenium.common.exceptions import (
    ...
    TimeoutException, InvalidElementStateException, StaleElementReferenceException,
)
from selenium.webdriver.common.by import By
from selenium.webdriver.common.action_chains import ActionChains
from selenium.webdriver.remote.webelement import WebElement as WebDriverElement
from selenium.webdriver.support.wait import WebDriverWait
from web.driver.exceptions import (ElementNotFound, ElementNotVisible, ElementStillVisible, ElementNotClickable,
                                   ElementNotInteractable, ElementAttributeError, ElementStillInteractable)
...
class WebElement(BaseElement):
            ...
            try:
                ...
                if attempt + 1 == self.wait_time:
                    raise ElementNotFound(
                        f"Элемент: {self} не найден в течение {self.wait_time} сек"
                    )
                time.sleep(1)
    ...
    def click(self) -> object:
        """В ГУ есть особенность некоторых кликов.
    ...
    def check_value(self, value):
        ...
        assert (
            element_value == value
        ), f"Значение `{element_value}` не сопадало с ожидаемым `{value}`"
    ...
    def wait_visible(self, wait_time=None):
        ...
        try:
            ...
            raise ElementNotVisible(
                f"Элемент: {self} не отображен в течение {self.wait_time} сек"
            )

    def wait_enabled(self):
        self._init()
        try:
            WebDriverWait(self.browser.driver, self.wait_time, ignored_exceptions=(StaleElementReferenceException, )).until(
                lambda driver: self.web_element.is_enabled()
            )
            return self
        except TimeoutException:
            raise ElementNotInteractable(
                f"Элемент: {self} не активен в течение {self.wait_time} сек"
            )
    ...
    def wait_not_visible(self):
        ...
        try:
            ...
            raise ElementStillVisible(
                f"Элемент: {self} отображается спустя {self.wait_time} сек"
            )
        except NoSuchElementException:
            return True
    ...
    def wait_disabled(self):
        self._init()
        try:
            WebDriverWait(self.browser.driver, self.wait_time).until(
                lambda driver: not self.web_element.is_enabled()
            )
            return self
        except TimeoutException as e:
            raise ElementStillInteractable(f"Элемент: {self._element} активен спустя {self.wait_time} сек {e}")
    ...
    def wait_value(self, value):
        ...
        while time.time() < timeout:
            ...
            raise ElementAttributeError(f"Не дождались значения {value} в элементе {self._element.xpath}")
...
class TextInput(WebElement):
    def __post_init__(self):
        self.xpath = (
            self.xpath
            or f"//input[@name='{self.name}'] | //textarea[@name='{self.name}'] | //input[@id='{self.name}']"
        )
...
class Label(WebElement):
    def __post_init__(self):
        self.xpath = (
            self.xpath or f"//label[contains(normalize-space(.),'{self.name}')]"
        )
